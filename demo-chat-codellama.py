...
with gr.Blocks() as demo:
    gr.Markdown("""<h1><center>Smart Assistant</center></h1>""")
    chatbot = gr.Chatbot()
    msg = gr.Textbox()
    state = gr.State()
    with gr.Row():
    ...
    def bot(history,temperature,top_p,slider_context_times):
        ...
        for i, n in enumerate(history):
            for j, m in enumerate(n):
                if m is not None:
                    messages.append({"role": "user" if j % 2 == 0 else "assistant", "content": m.replace('<br>','\n')})
        ...
        input_ids = tokenizer(prompt, return_tensors="pt",add_special_tokens=False).input_ids[:,-1024:].to('cuda') # mps/cuda
        ...
        for new_text in streamer:
            ...
            yield history
        end_time =time.time()
        print('Time：',end_time-start_time,'Length Message：',len(bot_message),'Time/length：',(end_time-start_time)/len(bot_message))
